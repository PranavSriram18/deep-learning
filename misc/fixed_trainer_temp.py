...
class BroadcastFunction(torch.autograd.Function):
    ...
    @staticmethod
    def backward(ctx, grad_output):
        # Sum gradients across all processes in group
...
class Mlp(nn.Module):
    ...
    def make_tp(self, rank: int, world_size: int) -> "Mlp":
        # Changes: handle swiglu sharding properly
...
def test(
    ...
    # print(f"tp rank: {info.tp_rank}")

    # print("Calling baseline grad_norm")
    baseline_gradnorm = grad_norm(model, dp_group)
    #print("Calling tp grad_norm")
